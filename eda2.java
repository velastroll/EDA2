...
public class eda2{
    ...
    public void add (int [] vector, int clave){
        if (almacen_vector.isEmpty()){
            almacen_vector.add(vector);
            almacen_claves.add(clave);
        } else {
            int posicion_almacen = search2add(vector);
            almacen_vector.add(posicion_almacen, vector);
            almacen_claves.add(posicion_almacen, clave);
        }
    }

    public int search2add(int [] vector2add){
        int [] vector = almacen_vector.get(posicion_almacen);
        if ( vector[posicion_vector] < vector2add[posicion_vector]) {
            posicion_almacen++;
            inicio = posicion_almacen;
        } else if ( vector[posicion_vector] == vector2add[posicion_vector]) {
            inicio = posicion_almacen;
            posicion_vector++;
            if (posicion_vector==vector.length){
    ...
    public ArrayList<Integer[]> almacen_vector = new ArrayList<Integer []>();
    public ArrayList<Integer> almacen_claves = new ArrayList<Integer>();

    public  int posicion_vector = 0;
    public  int inicio = 0;
    public  int posicion_almacen = inicio;
    public  int fin = almacen_vector.size();
    public  int evil_vector[] = {101, 118, 105, 108, 46,99, 111, 114, 112, 64, 109, 97, 100, 46, 111, 114, 103};
	public  int vPR[] = { 65, 54, 19, 98, 168, 33, 110, 187, 244, 22, 204, 4, 127, 100, 232, 93, 30, 242, 203, 42,
	...
	public  int vPI[] = { 71, 241, 180, 230, 11, 106, 114, 72, 133, 78, 158, 235, 226, 248, 148, 83, 224, 187,
	...
	public  int vPS[] = { 20, 83, 15, 86, 179, 200, 122, 156, 235, 101, 72, 23, 22, 21, 159, 2, 204, 84, 124, 131,
